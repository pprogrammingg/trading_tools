#!/usr/bin/env python3
"""
Timeframe Performance Backtest
Compare 2D, 1W, 2W, 1M timeframes to see which performs best
"""
...
def resample_to_timeframe(df, timeframe):
    ...
    if timeframe == "2D":
        ...
        df_resampled = df.resample('2W').agg({
            ...
            df_resampled = df.resample('ME').agg({  # Use 'ME' instead of deprecated 'M'
            'open': 'first',
            'high': 'max',
            'low': 'min',
            'close': 'last',
...
def main():
    """Run timeframe performance comparison"""
    print("\n" + "="*80)
    print("TIMEFRAME PERFORMANCE BACKTEST")
    print("Comparing 2D, 1W, 2W, 1M timeframes")
    print("="*80 + "\n")
    ...
    for timeframe in timeframes:
        ...
        if len(timeframe_moves) > 0:
            catch_rate = (sum(1 for m in timeframe_moves if m['score_at_entry'] >= 4) / len(timeframe_moves)) * 100
            if catch_rate > best_catch_rate:
                best_catch_rate = catch_rate
                best_timeframe = timeframe
    
    if best_timeframe:
        print(f"Best Catch Rate: {best_timeframe} ({best_catch_rate:.1f}%)")
    ...
    with open('timeframe_performance_results.json', 'w') as f:
        json.dump(output, f, indent=2, default=str)
    
    print(f"\n✓ Results saved to timeframe_performance_results.json")
