...
from result_score_access import get_ta_block, ta_float
...
def _load_esg_ratings(symbols: Set[str], cache_dir: Path, max_age_days: int = 7, allow_network: bool = True) -> Dict[str, Optional[str]]:
    ...
    try:
        ...
        from yfinance_quiet import quiet_yfinance, symbol_skips_yahoo_fundamentals
    except ImportError:
        return {s: None for s in symbols}
    with quiet_yfinance():
        for sym in list(symbols)[:400]:
            if symbol_skips_yahoo_fundamentals(sym):
                out[sym] = None
                continue
            try:
    ...
    for category, path in result_files:
        ...
        for symbol, symbol_data in data.items():
            ...
            for tf, key in [("1M", "1M_btc"), ("2W", "2W_btc"), ("1M", "1M_eth"), ("2W", "2W_eth")]:
                ...
                ta = get_ta_block(symbol_data, tf, denom_key)
                sc = ta_float(ta, "score")
                scores[key] = round(sc, 1) if sc is not None else None
...
def _compute_phase_perf(
    ...
    from yfinance_quiet import to_float
    ...
    for cat, syms in symbols_by_category.items():
        ...
        for s in syms:
            pv = perf.get(s) or {}
            phase_to_vals[phase_id].append(
                (to_float(pv.get("1M_vs_spy")), to_float(pv.get("1W_vs_spy")))
            )
    ...
    def _rank_key(val: Optional[float]) -> float:
        f = to_float(val)
        return f if f is not None else -999.0

    def rank_temp_for_period(use_1m: bool) -> Dict[str, str]:
        key = 1 if use_1m else 2
        sorted_phases = sorted(
            phase_avgs,
            key=lambda x: (_rank_key(x[key]) <= -999.0, -_rank_key(x[key])),
        )
        return {pid: rank_temps[min(i, len(rank_temps) - 1)] for i, (pid, _, _) in enumerate(sorted_phases)}
...
def write_index_with_cme(
    ...
    result_scores_dir = Path(__file__).resolve().parent / "result_scores"
    try:
        from index_landing_builder import build_index_primary_html

        primary_html = build_index_primary_html(result_scores_dir, allow_network=False)
    except Exception:
        primary_html = '    <p class="subtitle">Sector tops unavailable—run technical_analysis.py then visualize_scores --index-only.</p>'
    ...
    index_toc = """    <nav class="index-toc" aria-label="On this page">
        ...
        <a href="#unicorn-growth">Unicorns</a>
        <a href="#sector-tops">Sector tops</a>
        <a href="#section-legacy">More views</a>
        <a href="#section-categories">All categories</a>
    </nav>
...
"""
    ...
    <p class="subtitle">Start with <strong>unicorn growth</strong> (pre-IPO) and <strong>top sector leaders</strong> by 7D / 1M / 2M scores. Legacy dashboards are collapsed below.</p>
{index_toc}
    <div class="index-landing">
{primary_html}
    <details class="index-fold index-legacy" id="section-legacy">
        <summary>More views &amp; quick links (collapsed)</summary>
        <div class="index-fold__body">
        <p class="subtitle">Trending industries, Halal funds, hot pick plan, CME, dividends, crypto trends, wealth phase, and more.</p>
    <div class="main-cards-grid main-cards-grid--three">
{main_cards_block}    </div>
{link_bar_html}        </div>
    </details>
    <details class="index-fold index-legacy" id="section-categories">
        <summary>{INDEX_SECTION_CATEGORY_HEADER}</summary>
        <div class="index-fold__body">
        <p class="subtitle">Open a category to view the full score page (all symbols, market cap, ESG, etc.).</p>
...
{index_body}
    ...
    # Legacy technical index redirects to trade_analysis/index.html
    redirect = """<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8">
<meta http-equiv="refresh" content="0;url=../../index.html">
<title>Redirect to Trade Index</title></head>
<body><p><a href="../../index.html">Open trade analysis index</a></p></body></html>"""
    index_path.write_text(redirect, encoding="utf-8")
    print(f"  ✓ Updated {index_path.name} (redirect → trade_analysis/index.html)")
    print(
