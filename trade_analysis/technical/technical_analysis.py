...
def main():
    ...
    if args.refresh and (not args.category or args.category == "cryptocurrencies"):
        try:
            from crypto_universe import resolve_crypto_symbols
            syms = resolve_crypto_symbols(use_network=True, force_refresh=True)
            print(f"Crypto universe refreshed ({len(syms)}): {', '.join(syms)}")
        except Exception as e:
            print(f"Warning: crypto universe refresh failed: {e}")
    ...
    # Process categories (skip index-excluded niches unless --category targets one explicitly)
    ...
    try:
        ...
        if str(_trade_root / "fundamentals") not in sys.path:
            sys.path.insert(0, str(_trade_root / "fundamentals"))
        from exclusion_policy import EXCLUDED_CATEGORIES as _EXCLUDED_CATS

        skip_categories = set(_EXCLUDED_CATS)
    except Exception:
        skip_categories = set()

    if not args.category and skip_categories:
        before = len(categories_to_process)
        categories_to_process = [c for c in categories_to_process if c not in skip_categories]
        skipped = before - len(categories_to_process)
        if skipped:
            print(
                f"ℹ️  Skipping {skipped} excluded categor{'y' if skipped == 1 else 'ies'} "
                f"(defense/financials/macro): "
                f"{', '.join(sorted(skip_categories & set(symbols_config.keys())))}"
            )
